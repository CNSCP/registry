...
**Still to come (the second session):** forwarding — non-`GET` on a Profile path relayed to canon with the caller's own credential, as a pipe, so tools inside a boundary need one URL; the MCP server's `save_unpublished` and `get_unpublished`; and `deploy/INSTANCE.md`'s two worked deployments, private and public. Then the cut-over of `cp.padi.io` from the 0.11.0 server.
